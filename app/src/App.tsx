	...
	return (
		<main style={{ maxWidth: 640, margin: "4rem auto", padding: "0 1rem", font: "16px/1.5 system-ui, sans-serif" }}>
			...
			<form onSubmit={submit} style={{ display: "flex", gap: ".5rem", margin: "1.5rem 0" }}>
				...
				<button type="submit" style={{ padding: ".55rem .9rem", borderRadius: ".5rem", border: 0, background: "#ea580c", color: "#fff", fontWeight: 600, cursor: "pointer" }}>
					sign it
				</button>
			</form>
			...
			{entries.map((e) => (
				<div key={`${e.at}:${e.sig}`} style={{ padding: ".8rem 0", borderTop: "1px solid #8882" }}>
					...
					<div style={{ opacity: 0.55, fontSize: ".8rem" }}>
						{e.at} · <span style={{ fontFamily: "ui-monospace, monospace", background: "#ea580c22", color: "#ea580c", padding: "0 .35rem", borderRadius: ".35rem" }}>✓ {e.sig}</span>
					</div>
				</div>
			))}
		</main>
